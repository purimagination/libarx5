#include "hardware/MotorLibrary.h"
#include "math_ops.h"
...
void RV_can_data_repack(uint32_t msgID, uint8_t* Data, int32_t databufferlen, uint8_t comm_mode)
{
    ...
    if (msgID == 0x7FF)
    {
        ...
        if ((Data[0] == 0xff) && (Data[1] == 0xFF))
        {
            ...
            motor_comm_fbd.motor_feedback = 0x01;
    ...
    else if (comm_mode == 0x00) // Response mode
    {
        ...
        else if (ack_status == 4) // response frame 4
        {
            if (databufferlen != 3)
                return;
            motor_comm_fbd.instruction_code = Data[1];
            motor_comm_fbd.motor_feedback = Data[2];
        }
        else if (ack_status == 5) // response frame 5
        {
            motor_comm_fbd.instruction_code = Data[1];
            if (motor_comm_fbd.instruction_code == 1 & databufferlen == 6) // get position
            {
            ...
            else if (motor_comm_fbd.instruction_code == 2 & databufferlen == 6) // get speed
            {
                rv_type_convert.buf[0] = Data[5];
                rv_type_convert.buf[1] = Data[4];
                rv_type_convert.buf[2] = Data[3];
            ...
            else if (motor_comm_fbd.instruction_code == 3 & databufferlen == 6) // get current
            {
                rv_type_convert.buf[0] = Data[5];
                rv_type_convert.buf[1] = Data[4];
                rv_type_convert.buf[2] = Data[3];
            ...
            else if (motor_comm_fbd.instruction_code == 4 & databufferlen == 6) // get power
            {
                rv_type_convert.buf[0] = Data[5];
                rv_type_convert.buf[1] = Data[4];
                rv_type_convert.buf[2] = Data[3];
            ...
            else if (motor_comm_fbd.instruction_code == 5 & databufferlen == 4) // get acceleration
            {
                rv_motor_msg[motor_id_t].acceleration = Data[2] << 8 | Data[3];
            }
            else if (motor_comm_fbd.instruction_code == 6 & databufferlen == 4) // get linkage_KP
            {
                rv_motor_msg[motor_id_t].linkage_KP = Data[2] << 8 | Data[3];
            }
            else if (motor_comm_fbd.instruction_code == 7 & databufferlen == 4) // get speed_KI
            {
                rv_motor_msg[motor_id_t].speed_KI = Data[2] << 8 | Data[3];
            }
            else if (motor_comm_fbd.instruction_code == 8 & databufferlen == 4) // get feedback_KP
            {
                rv_motor_msg[motor_id_t].feedback_KP = Data[2] << 8 | Data[3];
            }
            else if (motor_comm_fbd.instruction_code == 9 & databufferlen == 4) // get feedback_KD
            {
